...
export async function GET(
  ...
  let query = admin.from('design_cables').select('*').eq('design_id', designId).order('created_at', { ascending: true })
  if (areaId) { query = query.eq('area_id', areaId) }

  const { data, error } = await query
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({ cables: data ?? [] })
}
...
export async function DELETE(
  ...
  const { id: designId } = await params
  const itemId = request.nextUrl.searchParams.get('cable_id')
  if (!itemId) return NextResponse.json({ error: 'cable_id required' }, { status: 400 })

  const admin = createAdminClient()
  const { data: design } = await admin.from('designs').select('id').eq('id', designId).eq('org_id', user.org_id).single()
  if (!design) return NextResponse.json({ error: 'Design not found' }, { status: 404 })

  const { error } = await admin.from('design_cables').delete().eq('id', itemId).eq('design_id', designId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
